function sumGreenTimeCells() {
    ...
    if (greenCellTimes.length > 0) {
      ...
      var outputRow = lastRow + 2;  // Next available row
      var outputCell = sheet.getRange(outputRow, 4); // Get the cell in column D of the next available row
      outputCell.setValue(totalTimeFormatted);  // Set the formatted total time
      outputCell.setBackground('#0000FF');  // Set the background color to blue
      outputCell.setFontColor('#FFFFFF');  // Optional: set font color to white for better visibility
    } else {
